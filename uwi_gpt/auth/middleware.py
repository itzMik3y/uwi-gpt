        ...
        try:
            ...
            if authorization and authorization.startswith('Bearer '):
                ...
                try:
                    ...
                    if request.state.token_type == 'refresh' and not request.url.path.endswith('/refresh'):
                        return JSONResponse(
                            status_code=status.HTTP_401_UNAUTHORIZED,
                            content={"detail": "Cannot use refresh token for this endpoint"}
                        )
                    
                    # Verify token in database
                    db = request.app.state.db_pool
                    token_record = await verify_token(db, token, request.state.token_type)
                    if not token_record:
